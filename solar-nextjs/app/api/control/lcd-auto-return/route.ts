...
export async function POST(request: Request) {
  try {
    ...
    console.log(`Attempting to set LCD auto return to: ${enabled}`);
    
    return NextResponse.json({ 
      success: true, 
      message: `LCD auto return ${enabled ? 'enabled' : 'disabled'}`,
      note: 'This is a read-only dashboard. Use WatchPower app to change settings.'
    });
  } catch (error: any) {
    console.error('Error in /api/control/lcd-auto-return:', error.message);
